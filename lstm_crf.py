...
parser.add_argument('--trainset', type=str, default=os.path.join('data', 'train.csv'), metavar='TRAINSET',
                    help='trainset path')
parser.add_argument('--devset', type=str, default=os.path.join('data', 'test.csv'), metavar='devset',
                    help='devset path')
...
def main(args):
  ...
  devset_loader = DataLoader(devset, batch_size=args.dev_batch_size, shuffle=False, num_workers=1, pin_memory=True)

  print("Building model")
  model = BiLSTMCRF(len(token2id), len(tag2id), args.embedding_size, args.hidden_size, args.rnn_layer, args.dropout,
  ...
  def _evaluate(if_report = False):
    ...
    with torch.no_grad():
      for bidx, batch in enumerate(devset_loader):
        seq = _prepare_data(batch[0], token2id, PAD, device)
